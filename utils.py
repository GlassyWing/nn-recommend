...
def random_generate_records(num_records: int, num_pairs: int, users: pd.DataFrame, comps: pd.DataFrame) -> pd.DataFrame:
    """
    随机生成构件使用纪录
    ...
    return pd.DataFrame(records, columns=['userId', 'compId', 'followCompId'])
